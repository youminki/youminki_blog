import { BlogPost, getUniqueId } from '../../src/types';
...
export const BLOG_POSTS: BlogPost[] = [
  createBlogPost({
    ...
    summary: '1. 폴링이란 무엇인가 폴링(Polling)은 클라이언트(브라우저, 앱 등)가 주기적으로 서버나 특정 상태를 확인하는 방식이다. 즉, 일정 간격마다 “상태가 바뀌었는가?”를 묻는 기법이다. 짧은 폴링(Short Polling): setInterval 같은 타이머를 이용해 일정 주기로 요청을 반복한다. 긴 폴링(Long Polling): 요청을 서버가 오래 ',
  ...
  createBlogPost({
    id: 4,
    title: '2만개 데이터로부터 12초 → 4초로 LCP 최적화하기: React 성능 최적화의 모든 것',
    ...
    summary: '2만개 크롤링 데이터로부터 12초 → 4초로 LCP 최적화하기: 실제 프로젝트에서 겪은 성능 문제들과 해결 과정 12초의 느린 렌더링 속도 프로젝트 초기, 우리는 2만개의 크롤링 데이터를 한 번에 렌더링하는 방식으로 개발을 시작했습니다. 사용자가 홈페이지에 접속하면 모든 상품 데이터를 동시에 로드하고 렌더링하는 방식이었죠. Largest Contentful',
    category: 'React',
    postType: 'custom',
    tags: ['React'],
  }),
  createBlogPost({
    id: 3,
    title: 'React에서 JWT 토큰 관리의 함정과 해결: 멜픽 개발 1년간의 실전 경험담',
    customDate: '2025.09.15.',
    ...
    summary: '�� 문제의 시작: 멜픽 개발 초기 토큰 관리의 혼란 처음에는 단순하게 생각했습니다. "토큰을 localStorage에 저장하고, 만료되면 갱신하면 되지 않을까?"라고요. 하지만 현실은 달랐습니다. iOS 사용자 불만: "앱이 자꾸 로그아웃돼요!" 개발팀 혼란: "왜 갑자기 401 에러가 폭발하는 거죠?" 사용자 이탈: 토큰 문제로 인한 사용자 이탈률 증가',
    category: 'React',
    postType: 'custom',
    tags: ['React'],
  }),
  createBlogPost({
    ...
    summary: '멜픽 개발 1년간 겪은 TypeScript 타입 에러들 처음에는 JavaScript로 개발하다가 TypeScript로 마이그레이션하면서 겪은 타입 에러들과 그 해결 과정을 정리해보겠습니다. 가장 자주 마주쳤던 에러들: Generic 타입 추론 실패 Union 타입 처리의 복잡성 any 타입 남발로 인한 타입 안전성 상실 전역 타입 선언의 중복 문제 실제 겪',
    category: 'TypeScript',
    postType: 'custom',
    tags: ['TypeScript'],
  }),
  createBlogPost({
    ...
    summary: '무한 스크롤 메모리 누수 문제들 멜픽 프로젝트에서 2만개의 상품 데이터를 효율적으로 보여주기 위해 무한 스크롤을 구현했을 때였습니다. 처음에는 단순하게 생각했습니다. "IntersectionObserver로 스크롤을 감지하고, 더 많은 아이템을 렌더링하면 되지 않을까?"라고요. 하지만 현실은 달랐습니다. 메모리 사용량: 2시간 사용 후 1.2GB까지 증가 ',
    category: 'React',
    postType: 'custom',
    tags: ['React'],
  }),
